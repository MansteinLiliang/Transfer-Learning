...
class NASM(Model):
    ...
    def __init__(self, sess, embed_dim=50, batch_size=64, vocab_size=4000,
                 h_dim=50, learning_rate=0.001, epoch=50, rnn_dim=200,
        ...
        self.dev_res = 0.0
        self.is_KL = False
        self.best_res = tf.Variable(0, trainable=False, dtype=tf.float32)
        self._attrs = ["embed_dim", "h_dim", "learning_rate", "dev_res", "is_KL"]
        self.build_model()
    ...
    def build_model(self):
        ...
        if self.is_KL:
            ...
            self.loss = tf.reduce_mean(0.0 * self.KL_lr * self.e_loss + self.c_loss)
    ...
    def mse(self):
        with tf.variable_scope('mse'):
            ...
            self.y_ = linear(self.context_vector, 1, bias=True, scope_name='pred')
    ...
    def attention(self):
        with tf.variable_scope("attention"):
            ...
            seq_len_mask = array_ops.sequence_mask(self.sequence_length, time_steps, dtype=tf.float32)
            ...
            seq_len_mask = tf.reshape(
                seq_len_mask,
                tf.reshape(
                    tf.concat([tf.reshape(tf.shape(seq_len_mask), [1, -1]),
                              tf.reshape(extra_ones, [1, -1])], 1), [-1])
            )
    ...
    def get_pred(self, x):
        y_ = self.sess.run(
            self.pred, feed_dict={self.x: x}
        )
        y_ = y_.flatten()
        return y_

    def train(self, U, train_batch, evl, dev_x):
        ...
        for idx in range(300000):
            ...
            if cur_epoch > epoch:
                ...
                # Developing
                pred = self.get_pred(dev_x).flatten()
                res = evl.feature_evaluate(pred)[0]  # dev res
                if res > self.best_res.eval():
                    self.dev_res = res
                    self.sess.run(self.best_res.assign(res))
                    self.save_best(self.checkpoint_dir)
